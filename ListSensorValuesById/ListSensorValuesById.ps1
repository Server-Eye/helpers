<# List All Sensors (and the last message of the sensor) By SensorID 
AUTOR: Mike Semlitsch
DATE: 07.10.2016
VERSION: V1.0
DESC: Creates an excel file with a report of all sensors (and the last message of the sensor) specified by the sensorID. The script iterates over all customers and sensorhubs to create the list.
#>
...
$Objworkbook.ActiveSheet.Cells.Item(1,5) = "LastMessage";
                ...
                if ( $sensorhub.subtype -eq "2" -and $sensorhub.parentId -eq $container.id) {
                    ...
                    :inner3 foreach($agent in $arrayAgents)
                    {
                        ...
                        if ($agent.subtype -like $subtypeOfAgent ) {
                            ...
                            Write-Host "             Message: " $agent.message;
