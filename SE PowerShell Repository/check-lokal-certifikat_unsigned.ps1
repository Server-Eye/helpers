...
if( [string]::IsNullOrEmpty($Thumbprint) -or !$days){
    ...
    if ($CertToCheck -eq $null){
        ...
        $api.setChartErrorLine($days) #set red error line in OCC chart, for example 80%
        $api.setMeasurementValue("Valid days remaining",$time)#measurement that will be added with name and DOUBLE value to the OCC chart
