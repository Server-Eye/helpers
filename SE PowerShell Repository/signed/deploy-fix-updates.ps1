...
<#
    .SYNOPSIS
        Download and install Fix Update
        
    .DESCRIPTION
        Download and install Fix Update,kb5001565,kb5001566,kb5001567,kb5001568
...
$script:_LogFilePath = "{0}\ServerEye.Custom.DeployFixUpdate.log" -f $Logdir
#endregion Server-Eye Default

#region Script specific
$OCCFile = "service_cc.connector"
$Downloaddir = "{0}\downloads" -f $SEDataPath
$FileToRunpath = "wusa.exe"

$1803 = [PSCustomObject]@{
    Name = "kb5001565.msu"
    x86 = "http://download.windowsupdate.com/d/msdownload/update/software/updt/2021/03/windows10.0-kb5001565-x86_f87e104677d8b32eda140b953205ac8bb8dda1eb.msu"
    x64 = "http://download.windowsupdate.com/d/msdownload/update/software/updt/2021/03/windows10.0-kb5001565-x64_18a2f1393a135d9c3338f35dedeaeba5a2b88b19.msu"
}
$1909 = [PSCustomObject]@{
    Name = "kb5001566.msu"
    x86 = "http://download.windowsupdate.com/c/msdownload/update/software/updt/2021/03/windows10.0-kb5001566-x86_d0134617d527f736cb73e2073b4269642c68d4a4.msu"
    x64 = "http://download.windowsupdate.com/d/msdownload/update/software/updt/2021/03/windows10.0-kb5001566-x64_b52b66b45562d5a620a6f1a5e903600693be1de0.msu"
}
$2009 = [PSCustomObject]@{
    Name = "kb5001567.msu"
    x86 = "http://download.windowsupdate.com/d/msdownload/update/software/updt/2021/03/windows10.0-kb5001567-x86_bafeb1bea06a5f39976de3406d3e33fb3cc2c6fe.msu"
    x64 = "http://download.windowsupdate.com/d/msdownload/update/software/updt/2021/03/windows10.0-kb5001567-x64_e3c7e1cb6fa3857b5b0c8cf487e7e16213b1ea83.msu"
}
$1809 = [PSCustomObject]@{
    Name = "kb5001568.msu"
    x86 = "http://download.windowsupdate.com/c/msdownload/update/software/updt/2021/03/windows10.0-kb5001568-x86_ac43cdb614bfc2b4692ab2503efddc041a5b0c02.msu"
    x64 = "http://download.windowsupdate.com/d/msdownload/update/software/updt/2021/03/windows10.0-kb5001568-x64_cbfb9504eda6bf177ad678c64b871a3e294514ce.msu"
}
$MaxAge = 604800000
...
if($releaseID -eq "2004"){
    $releaseID = "2009"
}
$Release = Get-Variable -Name $releaseID
[System.Uri]$URI = $Release.Value.$ARCHI
$URITMP = [System.Text.Encoding]::UTF8.GetBytes($URI.AbsoluteUri)
$Base64DownloadURL = [Convert]::ToBase64String($URITMP)
$DownloadFile = "{0}\{1}" -f $Downloaddir, $Release.Value.Name
...
if (!(Test-Path $DownloadFile)) {
    ...
    try {
    Get-SEFile -Base64DownloadURL $Base64DownloadURL -URL $URI.AbsoluteUri -MaxAge $MaxAge -Filename $Release.Value.Name -Filepath $DownloadFile
    }
    catch {
    Write-Error "$_"    
    }
...
$argument = "$DownloadFile /quiet /norestart /log:$($env:TEMP)\$($Release.Value.Name).log"
#region Arguments
$startProcessParams = @{
    ...
    Wait         = $true
    NoNewWindow  = $true
    RedirectStandardError = $script:_LogFilePath
}
...
try {
    ...
    try {
        ...
        Write-Log -Source $EventSourceName -EventID 3002 -EntryType Error -Message "Something went wrong $_ "
        Write-Log -Source $EventSourceName -EventID 3002 -EntryType Error -Message "More to find in $($env:TEMP)\$($Release.Value.Name).log"
    }
}
catch {
    Write-Log -Source $EventSourceName -EventID 3002 -EntryType Error -Message "Something went wrong $_ "
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQ